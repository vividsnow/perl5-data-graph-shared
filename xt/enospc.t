...
my ($mnt, @args) = @ARGV;
my $h = eval { Data::Graph::Shared->new("$mnt/x.shm", @args) };
print $h ? "created\n" : "refused: $@";
P
close $fh;
...
my ($sig, $out) = on_small_tmpfs('', 100, 500);
is $sig, 0, 'a segment that fits: no signal';
like $out, qr/^created/, '  and it is created';

my @big = (500_000, 500_000);   # about 14 MiB of node and edge arrays
...
($sig, $out) = on_small_tmpfs('DATA_GRAPH_SHARED_SPARSE=1', @big);
is $sig, POSIX::SIGBUS(), 'DATA_GRAPH_SHARED_SPARSE=1 skips the reservation: zeroing the segment faults';
