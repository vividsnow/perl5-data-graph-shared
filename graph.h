...
static GraphHandle *graph_create_memfd(const char *name, uint32_t max_nodes, uint32_t max_edges,
    ...
    uint64_t total = graph_total_size(max_nodes, max_edges);
    int fd = memfd_create(name ? name : "graph", MFD_CLOEXEC);
...
static GraphHandle *graph_open_fd(int fd, char *errbuf) {
    ...
    if (hdr->magic != GRAPH_MAGIC || hdr->version != GRAPH_VERSION ||
        hdr->total_size != (uint64_t)st.st_size ||
        hdr->max_nodes == 0 || hdr->max_edges == 0 ||
        hdr->max_nodes > 0x7FFFFFFFu || hdr->max_edges > 0x7FFFFFFFu ||
        hdr->total_size != graph_total_size(hdr->max_nodes, hdr->max_edges)) {
        GRAPH_ERR("invalid graph"); munmap(base, ms); return NULL;
    }
    /* Verify per-section offsets match expected layout */
    {
        ...
        if (hdr->node_data_off != exp_node_data ||
            hdr->node_heads_off != exp_node_heads ||
            hdr->node_bitmap_off != exp_node_bitmap ||
            hdr->edge_data_off != exp_edge_data ||
            hdr->edge_bitmap_off != exp_edge_bitmap) {
            GRAPH_ERR("invalid graph layout"); munmap(base, ms); return NULL;
        }
    }
