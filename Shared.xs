...
sync(self)
  ...
  PREINIT:
    EXTRACT_GRAPH(self);
  CODE:
    if (graph_msync(h) != 0) croak("graph sync: %s", strerror(errno));
...
eventfd(self)
  ...
  CODE:
    if (h->notify_fd < 0) {
        int efd = graph_create_eventfd(h);
        if (efd < 0) croak("eventfd: %s", strerror(errno));
    }
    RETVAL = h->notify_fd;
  OUTPUT:
    RETVAL
...
neighbors(self, node)
  ...
  PPCODE:
    ...
    while (eidx != GRAPH_NONE) {
        ...
        XPUSHs(sv_2mortal(newRV_noinc((SV *)pair)));
